...
Write-Host "=== Summary ===`nMFA Disabled (Licensed + Account Enabled): $($disabledMFAusers.count)"
    \end{minted}
    \caption[functie7.ps1]{Microsoft Graph PowerShell-script dat het aantal gelicenseerde, actieve accounts waarbij MFA niet aanwezig is weergeeft.}
    \label{PSQ7}
\end{listing}
...
Write-Host "=== Summary ===`nMFA Disabled (Guest): $($results.count)"
    \end{minted}
    \caption[functie8.ps1]{Microsoft Graph PowerShell-script dat het aantal externe accounts waarbij MFA uitstaat weergeeft.}
    \label{PSQ8}
\end{listing}
...
Write-Host "---`nGRAND TOTAL: $($results.count)"
    \end{minted}
    \caption[functie9.ps1]{Microsoft Graph PowerShell-script dat een overzicht geeft van de accounts met administrator-priviliges, onderverdeeld in gebruik van MFA.}
    \label{PSQ9}
\end{listing}
...
Write-Host "Temporary Access: $($tempaccessCount)`nHello for Business: $($hellobusinessCount)"
    \end{minted}
    \caption[functie20.ps1]{Microsoft Graph PowerShell-script dat een overzicht geeft van de verschillende MFA-methodes met het aantal gebruikers.}
    \label{PSQ20}
\end{listing}
