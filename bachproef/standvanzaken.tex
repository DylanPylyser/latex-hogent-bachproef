...
Vooraleer de autorisatie kan plaatsvinden, moet er eerst worden geauthenticeerd. Dit gebeurt via een tool genaamd \ac{ADAL} of de modernere \ac{MSAL} van \textcite{Microsoft2022d}. \ac{ADAL} en \ac{MSAL} dienen voor verificatie- en autorisatiefunctionaliteiten \autocite{Ooms2022}. Beide technologieën kunnen authenticeren met de Graph \ac{API}. \\ 
...
Een voorbeeld van reponse headers binnen Azure \ac{AD} Graph wordt weergegeven bij Figuur \ref{rhaad}. \\
...
Request bodies kunnen via \Ac{JSON}- of \ac{XML}-payloads verzonden worden voor POST-, PATCH- en PUT-\newline verzoeken \autocite{Microsoft2015}. Bovendien kunnen antwoorden, bijvoorbeeld van een server, worden teruggestuurd via \ac{JSON} of \ac{XML}. \\
...
Vervolgens wordt de verkregen token gebruikt om Microsoft Graph op te roepen. Het toegangstoken wordt in de autorisatieheader van een verzoek geplaatst \autocite{Microsoft2023r}. Dit is gelijkaardig aan de voorgestelling op Figuur \ref{MSGA}. \\
...
De tweede manier om een access token te verkrijgen, is zonder het gebruik van een user. Hieronder volgen de vijf stappen om dit te hanteren volgens \textcite{Microsoft2023s}. \\
...
Als eerste de OData-queryparameters. Deze queryparamaters zijn afkomstig van het OData-protocol waarvan bepaalde parameters worden ondersteund door \textcite{Microsoft2023g}. Een toegepast voorbeeld van een OData-queryparameter in een verzoek, is te zien op Figuur \ref{TRAM}. \\
