...
\subsection{Wat is de link tussen Azure AD en PowerShell-modules?}

% TODO... navragen of het interessant is om dit onderdeel te kaderen
...
% TODO: Klopt dit??? Navragen Jarne

Microsoft Graph bestaat uit drie componenten. \\

Het eerste component is de Microsoft Graph \Ac{API}. Dit is een eindpunt dat kan aangesproken worden via “https://graph.microsoft.com” dat Microsoft-entiteiten binnen de cloud kan aanspreken. \\

Het tweede component is Microsoft Graph connectors. Deze Graph-connectoren werken in de ingaande richting. Door het gebruik van deze connectoren, kunnen gegevens buiten de Microsoft-cloud worden gestuurd naar Graph en bijhorende toepassingen. \\

Als laatste worden er een set van tools aangeboden via Microsoft Graph Data Connect. Deze tools kunnen gebruikt worden om toepassingen te maken. De tools dienen voor het stroomlijnen van Graph-data naar Azure-datastores. \\
...
\subsubsection{Registreren van een applicatie op het Microsoft identity platform}

Een applicatie kan een token krijgen wanneer het geregistreerd is in het Azure-portaal. Wanneer de applicatie geregistreerd is, gaat het Microsoft-identiteitsplatform instaan voor volgende informatie:
...
Een voorbeeld van hoe een tokenverzoek er kan uitzien, is te vinden op Figuur \ref{HTR}. \\

\begin{figure}[h]
    \footnotesize\begin{verbatim}POST /common/oauth2/v2.0/token HTTP/1.1
...
&client_secret=JqQX2PNo9bpM0uEihUPzyrh
    \end{verbatim}    
    \caption[Voorbeeld tokenverzoek Microsoft Graph]{Een voorbeeld van een tokenverzoek via \ac{HTTP} binnen Microsoft Graph door \autocite{Microsoft2022b}.}
    \label{HTR}
\end{figure}
...
De andere drie methodes (POST, PATCH en PUT) gebruiken wel een request body. Meestal wordt in dit deel van het verzoek gebruikgemaakt van het \ac{JSON}-formaat. In dit formaat wordt er extra informatie meegeven over welke waarden moeten gebruikt worden voor een operatie of methode uit te voeren.
...
Mogelijke bronnen zijn bijvoorbeeld “me”, een user, een groep, etc. Deze bronnen ondersteunen ook relaties, waardoor andere onderdelen aangesproken kunnen worden. Ter illustratie, wanneer er een gebruiker een mail wilt sturen, is dit mogelijk via “me/sendMail”. \\
